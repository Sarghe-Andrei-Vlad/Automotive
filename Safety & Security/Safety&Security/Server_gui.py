...
class Ui_MainWindow(object):
    ...
    def send_key_data(self):
        unlock_car_encrypted = rsa_library.encrypt(self.pub_key, unlockCar)
        conexiune.send(cPickle.dumps(unlock_car_encrypted))
    ...
    def recv_messages_handler(self, stop_event):
        while stop_event:
            msg = cPickle.loads(conexiune.recv(1024))
            ...
            if not rsa_library.low_check(msg):
                conexiune.send(cPickle.dumps(rsa_library.encrypt(self.pub_key, hex(0))))
                flag_low = False
                flag = False
            elif not rsa_library.number_check(msg):
                conexiune.send(cPickle.dumps(rsa_library.encrypt(self.pub_key, hex(1))))
                flag = False
            else:
                conexiune.send(cPickle.dumps(rsa_library.encrypt(self.pub_key, hex(2))))
                flag = True
                flag_low = True
